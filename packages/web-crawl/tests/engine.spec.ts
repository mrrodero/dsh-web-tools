...
import { crawl, domainKey, normalizeUrl } from '../src/engine.ts'
import { EMPTY_POLICY } from '../src/robots.ts'
import type { CrawlOptions, PageFetcher, RawPage } from '../src/types.ts'
...
describe('crawl', () => {
  ...
  it('skips robots-disallowed URLs', async () => {
    const { parseRobots } = await import('../src/robots.ts')
