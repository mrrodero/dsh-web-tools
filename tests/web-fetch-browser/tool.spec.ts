...
import { formatRenderOutput, renderMetaFromResult, renderMetaFromValue } from '../../src/web-fetch-browser/tool.ts'
import type { WebRenderValue } from '../../src/web-fetch-browser/types.ts'
...
describe('renderMetaFromValue', () => {
  ...
  it('reports truncation when the body exceeds the cap', () => {
    const meta = renderMetaFromValue(value('x'.repeat(5_000)), 1_000) as { truncated: boolean }
